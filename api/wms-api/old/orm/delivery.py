...
class Body(Base):
    ...
    head_id = Column(ForeignKey('delivery.head.id', ondelete='CASCADE', onupdate='CASCADE'), primary_key=True, nullable=False)
...
class Head(Base):
    ...
    id = Column(BigInteger, primary_key=True, server_default=text("nextval('delivery.head_id_seq'::regclass)"))
