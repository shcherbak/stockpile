...
class Body(Base):
    __tablename__ = 'body'
    __table_args__ = {'schema': 'adjustmentcredit'}

    head_id = Column(ForeignKey('adjustmentcredit.head.id', ondelete='CASCADE', onupdate='CASCADE'), primary_key=True, nullable=False)
...
class Head(Base):
    ...
    __table_args__ = (
        ...
        {'schema': 'adjustmentcredit'}
    )

    id = Column(BigInteger, primary_key=True, server_default=text("nextval('adjustmentcredit.head_id_seq'::regclass)"))
