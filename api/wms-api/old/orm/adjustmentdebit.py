...
class Body(Base):
    __tablename__ = 'body'
    __table_args__ = {'schema': 'adjustmentdebit'}

    head_id = Column(ForeignKey('adjustmentdebit.head.id', ondelete='CASCADE', onupdate='CASCADE'), primary_key=True, nullable=False)
...
class Head(Base):
    ...
    __table_args__ = (
        ...
        {'schema': 'adjustmentdebit'}
    )

    id = Column(BigInteger, primary_key=True, server_default=text("nextval('adjustmentdebit.head_id_seq'::regclass)"))
