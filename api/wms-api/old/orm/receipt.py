...
class Body(Base):
    __tablename__ = 'body'
    __table_args__ = {'schema': 'receipt'}

    head_id = Column(ForeignKey('receipt.head.id', ondelete='CASCADE', onupdate='CASCADE'), primary_key=True, nullable=False)
...
class Head(Base):
    ...
    id = Column(BigInteger, primary_key=True, server_default=text("nextval('receipt.head_id_seq'::regclass)"))
