...
class Body(Base):
    __tablename__ = 'body'
    __table_args__ = {'schema': 'rebound'}

    head_id = Column(ForeignKey('rebound.head.id', ondelete='CASCADE', onupdate='CASCADE'), primary_key=True, nullable=False)
...
class Head(Base):
    ...
    id = Column(BigInteger, primary_key=True, server_default=text("nextval('rebound.head_id_seq'::regclass)"))
