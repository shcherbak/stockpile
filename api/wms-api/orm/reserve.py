...
class Body(Base):
    __tablename__ = 'body'
    __table_args__ = {'schema': 'reserve'}

    head_id = Column(ForeignKey('reserve.head.id', ondelete='CASCADE', onupdate='CASCADE'), primary_key=True, nullable=False)
...
class Head(Base):
    ...
    id = Column(BigInteger, primary_key=True, server_default=text("nextval('reserve.head_id_seq'::regclass)"))
