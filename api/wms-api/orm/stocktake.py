...
class Body(Base):
    ...
    head_id = Column(ForeignKey('stocktake.head.id', ondelete='CASCADE', onupdate='CASCADE'), primary_key=True, nullable=False)
    ...
    diff_qty = Column(Numeric, server_default=text("0"))
...
class Head(Base):
    ...
    id = Column(BigInteger, primary_key=True, server_default=text("nextval('stocktake.head_id_seq'::regclass)"))
